  ...
  <div v-if="boolean" class="tile is-ancestor is-flex-wrap-wrap" :style="windowWidth < 770 ? 'margin-left: 5%; margin-right: 5%' : ''">
    <div class="header tile is-parent is-12 mt-5 has-text-centered" :style="styleHeader">
      <img class="bgImage tile is-child is-inline-block" :style="styleHeaderImg"
        ...
        :src="this.process.env.BACKEND_URL + post.attributes.media.data[0].attributes.url" />
...
export default {
  ...
  async mounted() {
    await axios.get(this.process.env.BACKEND_URL + '/api/publicaciones?populate=media&filters[id][$eq]=' + this.$route.params.id)
      .then((response) => {
          ...
          for (let i = 0; i < this.body.length; i++) {
            if (i > 1 && i % num == 0 && i < this.body.length - 2) {
              if (this.body[i].title) {
                this.body[i + 1].imgUrl = this.process.env.BACKEND_URL + this.post.attributes.media.data[imgIndex % this.post.attributes.media.data.length].attributes.url
                ...
                this.body[i].imgUrl = this.process.env.BACKEND_URL + this.post.attributes.media.data[imgIndex % this.post.attributes.media.data.length].attributes.url
